...
struct DrinkerPool 
{
	...
	/////////////////////////////////////////////////////////////////////////
	std::mutex stopDrinkingFlagMutex;
};
...
bool TryToGetResources(Drinker *currentDrinker)
{
	...
	for (int i = 0; i < currentDrinker->resourcePool->totalResources; i++) {
		...
		if (firstResource->type == ResourceType::Bottle && currentDrinker->resourcePool->resources[i].type == ResourceType::Opener && currentDrinker->resourcePool->resources[i].resourceMutex.try_lock()) {
			secondResource = &currentDrinker->resourcePool->resources[i];
	...
	for (int i = 0; i < currentDrinker->resourcePool->totalResources; i++) {
		if (currentDrinker->resourcePool->resources[i].type == ResourceType::Bottle && currentDrinker->resourcePool->resources[i].resourceMutex.try_lock()) {
			secondResource = &currentDrinker->resourcePool->resources[i];
	...
	/*for (int pos = 0; pos < currentDrinker->resourcePool->totalResources; pos++) {
		currentDrinker->resourceTryCount++;

		if (currentDrinker->resourcePool->resources[pos].resourceMutex.try_lock()) {
			//successful lock 

			//increment lock count
			currentDrinker->resourcePool->resources[pos].lockCount++;

			if (currentDrinker->bottle == nullptr && currentDrinker->resourcePool->resources[pos].type == ResourceType::Bottle) {
				currentDrinker->bottle = &currentDrinker->resourcePool->resources[pos];
				return true;
			}
			else if (currentDrinker->opener == nullptr && currentDrinker->resourcePool->resources[pos].type == ResourceType::Opener) {
				currentDrinker->opener = &currentDrinker->resourcePool->resources[pos];
				return true;
			}
		}
	}*/

	//release first resource if no second resource is locked
...
void StartDrinker(Drinker *currentDrinker)
{
	...
	while(true)
	{
		...
		currentDrinker->drinkerPool->stopDrinkingFlagMutex.lock();

		if(currentDrinker->drinkerPool->stopDrinkingFlag)
		{
			currentDrinker->drinkerPool->stopDrinkingFlagMutex.unlock();
			break;
		}
		currentDrinker->drinkerPool->stopDrinkingFlagMutex.unlock();
	}
...
void DrinkerThreadEntrypoint(Drinker *currentDrinker)
{
	...
	std::unique_lock<std::mutex> startingGun(currentDrinker->drinkerPool->startingGunMutex);
	currentDrinker->drinkerPool->startingGunCondition.wait(startingGun, [currentDrinker] { return currentDrinker->drinkerPool->startingGunFlag; });
	startingGun.unlock();
...
int main(int argc, char **argv)
{
	...
	std::unique_lock<std::mutex> waitForDrinkers(poolOfDrinkers.drinkerCountMutex);
	poolOfDrinkers.drinkerCountCondition.wait(waitForDrinkers, [&poolOfDrinkers, drinkerCount] { return poolOfDrinkers.drinkerCount == poolOfDrinkers.totalDrinkers; });
	waitForDrinkers.unlock();
	...
	//delay call to Pause to allow starting prints to print before the pause text is printed
	...
	printf("Main: Sending stop");
	poolOfDrinkers.stopDrinkingFlagMutex.lock();
	poolOfDrinkers.stopDrinkingFlag = true;
	poolOfDrinkers.stopDrinkingFlagMutex.unlock();
	...
	waitForDrinkers.lock();
	poolOfDrinkers.drinkerCountCondition.wait(waitForDrinkers, [&poolOfDrinkers] {return poolOfDrinkers.drinkerCount == 0; });
	waitForDrinkers.unlock();
